...
uninit-full: clear uninit
	rm -rf .idea java8-spring.iml .git
...
build:
	./mvnw verify
	chmod +x ./target/java8-spring-0.0.1-SNAPSHOT.jar

run:
	./mvnw spring-boot:start -Dspring.profiles.active=local
#	./target/java8-spring-0.0.1-SNAPSHOT.jar
#	java -jar --enable-preview ./target/java8-spring-0.0.1-SNAPSHOT.jar
...
git-fork-init: init
	git remote add upstream git://github.com/Vyacheslav-Lapin/java8-spring.git
	git fetch upstream
